...
import { getUserEmailFromDB, getUserIDFromDB, getUsernameFromDB, registerUser, resetPasswordById } from "../Services/auth.services.js";
import { generateHashedPassword } from "../Utility/hash.js";
import { sendPasswordResetMail } from "../Utility/sendmail.js";
import { forgotPasswordvalidation, loginValidation, resetPasswordValidation, signUpValidation } from "../Middleware/authValidation.js";
dotenv.config();
...
router.post("/signup", signUpValidation(), express.json(), async (request, response) => {
    ...
    // if username already exists throws an error
        ...
        const result = await registerUser({ username: username, email: email, password: hashedPassword })
...
router.post("/login", loginValidation(), express.json(), async (request, response) => {
    ...
    if (!userEmailFromDB) {
        ...
        if (isPasswordMatch) {
            const token = jwt.sign({ id: userEmailFromDB._id }, process.env.SECRET_KEY, { expiresIn: "2h" })
...
router.post("/forgot-password", express.json(), forgotPasswordvalidation(), async (request, response, next) => {
    ...
    if (userEmailFromDB === null) {
        response.status(400).send({ message: "Please enter the registered email" })
    }
    // else sends a **unique-one-time-link** to the user for resetting the password
        ...
        if (sendMail) {
            response.send({ message: `Password reset mail has been sent successfully to ${email}. Kindly check your spam folder also.` })
        }
        else {
...
router.post("/reset-password/:_id/:token", express.json(), resetPasswordValidation(), async (request, response, next) => {
    const { _id, token } = request.params
    ...
    if (userId !== _id) {
        ...
        try {
            ...
            const updatedResult = await resetPasswordById({ userId, password: hashedPassword })
